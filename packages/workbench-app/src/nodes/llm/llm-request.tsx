...
export const llmRequestNodeType: WorkflowRuntimeNodeTypeDefinition = {
  ...
  execute: async ({ inputs, data, controller }) => {
    ...
    const processStream = async (emit: (output: OllamaOutputs) => void): Promise<void> => {
      ...
      try {
        controller.setProgress({ progressRatio: 0.1, message: `Connecting to ${CONFIG.name}...` });
        emit({
          chunk: {},
          thought: undefined,
          response: undefined,
...
export const createLlmRequestNodes = (config: Partial<typeof CONFIG>) => {
  const finalConfig = { ...CONFIG, ...config };

  return {
    [`${finalConfig.typeSuffix}Streaming`]: llmRequestNodeType,
  };
};
