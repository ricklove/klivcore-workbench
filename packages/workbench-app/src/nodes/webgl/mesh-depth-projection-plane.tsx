...
export const threeMeshDepthProjectionPlane: WorkflowRuntimeNodeTypeDefinition =
  {
    ...
    execute: async ({ inputs, runtimeState }) => {
      ...
      const hasTextureChanged =
        texture !== rs.texture || depthTexture !== rs.depthTexture;
      ...
      const paramsChanged =
        JSON.stringify(currentParams) !== JSON.stringify(rs.cachedParams);
      ...
      const dims = getDims(depthTexture) ??
        getDims(texture) ?? { w: 512, h: 512 };
